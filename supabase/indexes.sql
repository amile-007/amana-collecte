...
-- IF NOT EXISTS = idempotent, safe à ré-exécuter
-- ============================================================

-- Demandes : filtrage par statut + centre (backoffice chef_centre)
CREATE INDEX IF NOT EXISTS idx_demandes_statut_centre
...
CREATE INDEX IF NOT EXISTS idx_demandes_collecteur_statut
  ON demandes(collecteur_id, statut, updated_at DESC);

-- Colis : jointure avec demandes
CREATE INDEX IF NOT EXISTS idx_colis_demande_statut
  ON colis(demande_id, statut);

-- Collecteurs : carte GPS + filtrage par centre/statut
CREATE INDEX IF NOT EXISTS idx_collecteurs_position
...
CREATE INDEX IF NOT EXISTS idx_statuts_historique_demande
  ON statuts_historique(demande_id, created_at DESC);

-- Anomalies : filtrage par centre via jointure collecteurs
CREATE INDEX IF NOT EXISTS idx_anomalies_demande
